# 🎨 Sora ChatGPT 자동 저장 확장 프로그램

Sora ChatGPT 라이브러리에서 이미지와 프롬프트를 자동으로 수집하는 Chrome 확장 프로그램 모음입니다.

## 📦 프로젝트 구조

### 🔄 **chrome-extension-test** - 기본 자동 저장 확장 프로그램
- Sora ChatGPT 라이브러리에서 이미지와 프롬프트 자동 수집
- 30초마다 자동 저장 (설정 가능)
- 실시간 카운트다운 표시
- JSON 형식 데이터 다운로드
- 간단하고 직관적인 인터페이스

### 🗂️ **auto-save-json-test** - 고급 자동 저장 확장 프로그램
- 기본 기능 + 고급 설정 옵션
- 더 상세한 데이터 수집
- 향상된 UI/UX
- 고급 로깅 및 오류 처리
- 확장된 설정 기능

## 🚀 빠른 시작

### 1. 기본 확장 프로그램 설치 (chrome-extension-test)
```bash
# Chrome 브라우저에서
1. chrome://extensions/ 접속
2. "개발자 모드" 활성화
3. "압축해제된 확장 프로그램을 로드합니다" 클릭
4. chrome-extension-test 폴더 선택
```

### 2. 고급 확장 프로그램 설치 (auto-save-json-test)
```bash
# Chrome 브라우저에서
1. chrome://extensions/ 접속
2. "개발자 모드" 활성화
3. "압축해제된 확장 프로그램을 로드합니다" 클릭
4. auto-save-json-test 폴더 선택
```

### 3. Sora ChatGPT 라이브러리 접속
```
🌐 필수: https://sora.chatgpt.com/library 페이지로 접속하세요!
```

## ✨ 주요 기능

### 🔄 자동 저장 시스템
- **30초마다 자동 저장**: 설정 가능한 간격 (10-3600초)
- **실시간 카운트다운**: 다음 저장까지 남은 시간 표시
- **중복 방지**: 이미 저장된 데이터는 건너뜀
- **실시간 감지**: 페이지 변경 시 자동으로 새 데이터 수집

### 📸 데이터 수집
- **이미지 수집**: URL, 크기, alt 텍스트, 메타데이터
- **프롬프트 수집**: 텍스트, 타임스탬프, 페이지 정보
- **JSON 형식**: 구조화된 데이터 저장

### 🎯 사용자 인터페이스
- **직관적인 팝업**: 쉬운 설정 및 제어
- **실시간 상태 표시**: 현재 작업 상태 확인
- **원클릭 다운로드**: 수집된 데이터 즉시 다운로드

## 📁 폴더 구조

```
sora-auto-image/
├── chrome-extension-test/          # 기본 자동 저장 확장 프로그램
│   ├── manifest.json              # 확장 프로그램 설정
│   ├── popup.html                 # 팝업 인터페이스
│   ├── popup.js                   # 팝업 로직
│   ├── content.js                 # 페이지 콘텐츠 스크립트
│   ├── background.js              # 백그라운드 스크립트
│   ├── i18n.js                    # 다국어 지원
│   ├── _locales/                  # 언어 파일
│   │   ├── en/messages.json      # 영어
│   │   └── ko/messages.json      # 한국어
│   └── data.json                  # 샘플 데이터
├── auto-save-json-test/            # 고급 자동 저장 확장 프로그램
│   ├── manifest.json              # 확장 프로그램 설정
│   ├── popup.html                 # 고급 팝업 인터페이스
│   ├── popup.js                   # 고급 팝업 로직
│   ├── content.js                 # 고급 콘텐츠 스크립트
│   ├── background.js              # 백그라운드 스크립트
│   ├── i18n.js                    # 다국어 지원
│   ├── _locales/                  # 언어 파일
│   │   ├── en/messages.json      # 영어
│   │   └── ko/messages.json      # 한국어
│   ├── icon_16.png               # 16x16 아이콘
│   ├── icon_48.png               # 48x48 아이콘
│   ├── icon_128.png              # 128x128 아이콘
│   └── README.md                 # 상세 사용법 가이드
├── .gitignore                     # Git 무시 파일 설정
├── README.md                      # 이 파일 (한국어)
└── README_EN.md                   # 영어 버전
```

## 🎯 사용 시나리오

### 시나리오 1: 간단한 자동 저장
1. **chrome-extension-test** 설치
2. **https://sora.chatgpt.com/library** 페이지 접속
3. 자동 저장 켜기
4. 30초마다 자동으로 데이터 수집

### 시나리오 2: 고급 데이터 수집
1. **auto-save-json-test** 설치
2. 고급 설정으로 자동 저장 간격 조정
3. 상세 로깅 및 모니터링
4. 향상된 데이터 수집 기능 활용

### 시나리오 3: 대량 데이터 수집
1. 고급 확장 프로그램 설치
2. 자동 저장 간격을 10초로 설정
3. 장기간 수집 실행
4. JSON 파일로 데이터 다운로드

## 📊 데이터 형식

### 이미지 데이터
```json
{
  "id": "img_1234567890_0",
  "url": "https://example.com/image.jpg",
  "alt": "이미지 설명",
  "width": 1024,
  "height": 768,
  "timestamp": "2025-01-27T12:00:00.000Z",
  "pageUrl": "https://sora.chatgpt.com/library"
}
```

### 프롬프트 데이터
```json
{
  "id": "prompt_1234567890_0",
  "text": "프롬프트 텍스트 내용",
  "timestamp": "2025-01-27T12:00:00.000Z",
  "pageUrl": "https://sora.chatgpt.com/library",
  "selector": "[data-testid=\"prompt-text\"]"
}
```

## ⚙️ 설정 옵션

### 자동 저장 간격
- **최소**: 10초 (빠른 수집)
- **기본**: 30초 (권장)
- **최대**: 3600초 (1시간, 느린 수집)

### 확장 프로그램별 특징

#### chrome-extension-test (기본)
- 간단한 설정
- 직관적인 인터페이스
- 빠른 시작

#### auto-save-json-test (고급)
- 상세한 설정 옵션
- 고급 UI/UX
- 향상된 로깅
- 더 많은 커스터마이징 옵션

## 🔧 고급 기능

### 실시간 모니터링
- **DOM 변경 감지**: MutationObserver 사용
- **페이지 변경 감지**: URL 변경 시 자동 대응
- **오류 처리**: 네트워크 오류 자동 복구

### 스마트 데이터 수집
- **중복 방지**: 동일한 데이터 자동 필터링
- **메타데이터 수집**: 상세 정보 포함
- **구조화된 저장**: JSON 형식으로 정리

### 사용자 경험
- **실시간 피드백**: 작업 상태 즉시 확인
- **원클릭 다운로드**: 쉬운 데이터 내보내기
- **반응형 디자인**: 다양한 화면 크기 지원

## 🐛 문제 해결

### 자동 저장이 작동하지 않는 경우
1. **페이지 URL 확인**: `https://sora.chatgpt.com/library`
2. **권한 확인**: 확장 프로그램 권한 허용
3. **페이지 새로고침**: F5 키로 페이지 다시 로드
4. **확장 프로그램 다시 로드**: chrome://extensions/에서 다시 로드

### 데이터가 수집되지 않는 경우
1. **콘솔 로그 확인**: F12 → Console 탭
2. **네트워크 연결**: 인터넷 연결 상태 확인
3. **페이지 로딩 대기**: 페이지가 완전히 로드될 때까지 대기

### 카운트다운이 표시되지 않는 경우
1. **자동 저장 ON 확인**: 토글 스위치가 ON인지 확인
2. **페이지 새로고침**: 페이지를 새로고침하여 콘텐츠 스크립트 다시 로드
3. **확장 프로그램 다시 로드**: 확장 프로그램을 다시 로드

## 📞 지원 및 기여

### 버그 신고
문제가 발생하면 다음 정보와 함께 이슈를 생성해 주세요:
- **브라우저 버전**: Chrome 버전
- **확장 프로그램 버전**: 설치된 확장 프로그램
- **오류 메시지**: 콘솔 로그 내용
- **재현 단계**: 문제 발생 과정

### 기능 제안
새로운 기능이나 개선사항:
- **사용 사례**: 언제 필요한지
- **예상 동작**: 어떻게 작동해야 하는지
- **우선순위**: 얼마나 중요한지

## 📄 라이선스

이 프로젝트는 MIT 라이선스 하에 배포됩니다.

## 🔄 업데이트 히스토리

### v2.4.0 (2025-01-27)
- ✅ 최종 폴더 구조 정리
- ✅ 확장 프로그램 관련 파일만 유지
- ✅ Python 도구 제거로 깔끔한 구조
- ✅ Chrome Web Store 등록 준비 완료
- ✅ 메인 README 파일 정리

### v2.3.0 (2025-01-27)
- ✅ 폴더 구조 정리
- ✅ 두 개의 확장 프로그램 분리
- ✅ 실행 파일 제거
- ✅ .gitignore 파일 추가

### v2.2.0 (2025-01-26)
- ✅ 실시간 카운트다운 기능
- ✅ 중복 방지 시스템
- ✅ JSON 파일 검증
- ✅ 오류 처리 개선

### v2.1.0 (2025-01-25)
- ✅ 자동 저장 기능
- ✅ 이미지/프롬프트 수집
- ✅ JSON 다운로드
- ✅ 기본 UI 구현

---

**개발자**: yoohyunseog  
**GitHub**: https://github.com/yoohyunseog/sora-chatgpt-downloader  
**지원 브라우저**: Chrome 88+  
**필수 페이지**: https://sora.chatgpt.com/library  
**최종 업데이트**: 2025-01-27 