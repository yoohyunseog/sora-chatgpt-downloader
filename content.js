...
function displayDuplicateWarning(result) {
  ...
  // 매치된 단어들을 표시 (최대 10개)
  const matchedWordsDisplay = result.matchedWords && result.matchedWords.length > 0 
    ? result.matchedWords.slice(0, 10).join(', ') + (result.matchedWords.length > 10 ? '...' : '')
    : '정보 없음';
  
  // 매치된 프롬프트를 50자로 제한하여 표시
  const matchedPromptDisplay = result.matchedPrompt && result.matchedPrompt.length > 50 
    ? result.matchedPrompt.substring(0, 50) + '...'
    : result.matchedPrompt || '정보 없음';
  
  // 중복률에 따른 색상 및 아이콘 설정
  let ratingIcon = '⚠️';
  let ratingColor = '#ff6b6b';
  if (result.matchRatio >= 90) {
    ratingIcon = '🔴';
    ratingColor = '#dc2626';
  } else if (result.matchRatio >= 80) {
    ratingIcon = '🟠';
    ratingColor = '#ea580c';
  } else if (result.matchRatio >= 70) {
    ratingIcon = '🟡';
    ratingColor = '#ca8a04';
  }
  
  warningDiv.innerHTML = `
    <h4>${ratingIcon} 중복 프롬프트 발견</h4>
    <div style="background: rgba(255,255,255,0.1); padding: 10px; border-radius: 4px; margin: 8px 0;">
      <div style="font-size: 18px; font-weight: bold; color: #fff200; margin-bottom: 5px;">
        📊 매칭률: ${result.matchRatio.toFixed(1)}%
      </div>
    ...
    <div style="font-size: 12px; margin: 8px 0;">
      <strong>매칭된 단어:</strong><br>
      <span style="background: rgba(255,255,255,0.2); padding: 2px 6px; border-radius: 3px; font-size: 11px;">
        ${matchedWordsDisplay}
      </span>
    </div>
    <div style="font-size: 12px; margin: 8px 0;">
      <strong>유사한 프롬프트:</strong><br>
      <div style="background: rgba(255,255,255,0.1); padding: 6px; border-radius: 3px; font-size: 11px; font-style: italic;">
        "${matchedPromptDisplay}"
      </div>
    </div>
    <div style="margin-top: 10px; text-align: center;">
      <button onclick="this.parentElement.remove()" 
              style="background: white; color: ${ratingColor}; border: none; padding: 6px 12px; border-radius: 4px; cursor: pointer; font-weight: bold;">
        확인
      </button>
    </div>
  `;
